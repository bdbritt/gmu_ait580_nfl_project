...
def load_data() -> pd.DataFrame:
    ...
    teams = pd.read_csv(os.path.join(PROJECT_DIR, TEAMS_FILE))
...
def get_season_stats(teams, start_year, end_year) -> None:
    """
    Get team stats
    """
    ...
    # team stats
    time_period = list(pd.date_range(start=start_year, end=end_year, freq=pd.DateOffset(years=1)).strftime('%Y'))
    ...
    stats_df.to_csv(os.path.join(PROJECT_DIR, TEAMS_STATS), index=False)


def main(start_date: Text, end_date: Text):

    logger = logging.getLogger(__name__)
    logger.info('getting data')

    teams_data = load_data()
    get_season_stats(teams_data, start_date, end_date)
